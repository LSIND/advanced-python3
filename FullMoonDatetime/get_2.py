...
def get_year(year, page):
    ...
    if resp.status_code == 200:
        ...
        for x in t.find_all('td'):
            if x.attrs == {'class': ['notes']} and x.text.isspace():   # all except notes
                all_moons.append('')
            else:
                all_moons.append(x.text.strip())
        print(all_moons)
        if all_moons is not None:
            all_moons = [x for x in zip(*[iter(all_moons)]*4)] # tuples ('Sunday', '15 April 1900', '02:02:06 am')
            print(year, write_to_csv(all_moons, 'fullmoons2.txt'))  # write to csv
